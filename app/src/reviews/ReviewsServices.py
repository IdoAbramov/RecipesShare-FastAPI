...
class ReviewsServices():
    ...
    def get_all_recipe_reviews_service(self, 
        ...
        reviews = reviews_repo.get_all_recipe_reviews_data(recipe_id, limit, skip)
        if not reviews:
    ...
    def get_recipe_review_by_user_id_service(self, 
        ...
        reviews_repo = ReviewsRepository()
        review = reviews_repo.get_recipe_review_data_by_user_id(user_id, recipe_id)
        if not review:
    ...
    def get_average_rating_service(self, 
        ...
        reviews_repo = ReviewsRepository()
        review_exist = reviews_repo.is_recipe_reviewed(recipe_id)
        if review_exist == None:
            raise ReviewsExceptions.RecipeReviewsNotFound(recipe_id)
        avg_rating = reviews_repo.get_recipe_average_rating_data(recipe_id)
    ...
    def create_new_review_service(self, 
        ...
        review_exist = reviews_repo.get_recipe_review_data_by_user_id(user_id, review.recipe_id)
        if review_exist:
        ...
        new_review = reviews_repo.create_new_recipe_review_data(review_model, user_id)
        return new_review
    ...
    def update_review_service(self, 
        ...
        review_exist = reviews_repo.get_recipe_review_data_by_user_id(current_user_id, review.recipe_id)
        if not review_exist:
            raise ReviewsExceptions.UserReviewNotFound(review.recipe_id, current_user_id)

        updated_review = reviews_repo.update_exist_recipe_review_data(review, current_user_id)

        return updated_review
    
    def delete_review_service(self, current_user_id: int, recipe_id: int) -> None:
        reviews_repo = ReviewsRepository()
        review_exist = reviews_repo.get_recipe_review_data_by_user_id(current_user_id, recipe_id)
        if not review_exist:
            raise ReviewsExceptions.UserReviewNotFound(recipe_id, current_user_id)
        reviews_repo.delete_exist_recipe_review_data(recipe_id, current_user_id)
